...
class TransportDSLAction @Inject()(settings: Settings,
  ...
  extends HandledTransportAction[DSLRequest, DSLResponse](settings,
    DSLAction.NAME,
    threadPool, transportService,
    actionFilters,
  ...
  override def doExecute(task: Task, dSLRequest: DSLRequest, listener: ActionListener[DSLResponse]): Unit = {
    val result = interpreter.run(dSLRequest.request)
    listener.onResponse(DSLResponse(result))
  }
  override def doExecute(request: DSLRequest, listener: ActionListener[DSLResponse]): Unit = {
