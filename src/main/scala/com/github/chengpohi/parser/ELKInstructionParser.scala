...
class ELKInstructionParser(elkCommand: ELKCommand, parserUtils: ParserUtils) extends CollectionParser {
  ...
  val query = P("query" ~ space ~/ ioParser ~ joinQuery.?).map(c => ("query", Some(elkCommand.query), c._1 ++ c._2.getOrElse(Seq())))
  val joinQuery = P("join" ~ space ~ ioParser ~ space ~ "by" ~ ioParser).map(c => c._1 ++ c._2)
