...
/**
  ...
  */
class ELKInstructionParser(val interceptFunction: InterceptFunction)
  ...
  val health = P("health").map(s =>
    Instruction("health", interceptFunction.health, Seq(Str(""))))
  val shutdown = P("shutdown").map(s =>
    Instruction("shutdown", interceptFunction.shutdown, Seq(Str(""))))
  val count = P("count" ~/ ioParser).map(c =>
    Instruction("count", interceptFunction.count, c))
  //memory, jvm, nodes, cpu etc
  val clusterStats = P("cluster stats").map(s =>
    Instruction("clusterStats", interceptFunction.clusterStats, Seq()))
  //indices, aliases, restore, snapshots, routing nodes etc
  val clusterState = P("cluster state").map(s =>
    Instruction("clusterState", interceptFunction.getIndices, Seq()))
  val indicesStats = P("indices stats").map(s =>
    Instruction("indiciesStats", interceptFunction.indicesStats, Seq()))
  val nodeStats = P("node stats").map(s =>
    Instruction("nodeStats", interceptFunction.nodeStats, Seq()))
  val clusterSettings = P("cluster settings").map(s =>
    Instruction("clusterSettings", interceptFunction.clusterSettings, Seq()))
  val nodeSettings = P("node settings").map(s =>
    Instruction("nodeSettings", interceptFunction.nodeSettings, Seq()))
  val indexSettings = P(ioParser ~ "settings").map(s =>
    Instruction("nodeSettings", interceptFunction.indexSettings, s))
  val pendingTasks = P("pending tasks").map(s =>
    Instruction("pendingTasks", interceptFunction.pendingTasks, Seq()))
  val waitForStatus = P("wait for status" ~ ioParser).map(s =>
    Instruction("pendingTasks", interceptFunction.waitForStatus, s))
  val deleteDoc = P(
    "delete" ~ "from" ~/ strOrVar ~ "/" ~/ strOrVar ~ "id" ~ strOrVar)
    .map(c =>
      Instruction("delete", interceptFunction.deleteDoc, Seq(c._1, c._2, c._3)))
  val deleteIndex = P("delete" ~ "index" ~/ strOrVar).map(c =>
    Instruction("delete", interceptFunction.deleteIndex, Seq(c)))

  val termQuery = P("term" ~ "query" ~/ ioParser).map(c =>
    Instruction("termQuery", interceptFunction.query, c))
  val joinSearch = P("join" ~ strOrVar ~ "/" ~ strOrVar ~ "by" ~ strOrVar)
    .map(
      c =>
        Instruction("joinQuery",
  ...
  val search: P[Instruction] = P(
    "search" ~ "in" ~/ strOrVar ~ ("/" ~ strOrVar ~ (matchQuery | joinSearch).?).?)
    .map(c =>
      c._2 match {
        case None => Instruction("query", interceptFunction.query, Seq(c._1))
        case Some((f, None)) =>
          Instruction("query", interceptFunction.query, Seq(c._1, f))
        case Some((f, Some(t))) =>
          Instruction(t.name, t.f, Seq(c._1, f) ++ t.params)
    })
  ...
  val bulkIndex = P("bulk index" ~/ ioParser).map(c =>
    Instruction("bulkIndex", interceptFunction.bulkIndex, c))
  val updateMapping = P("update mapping" ~/ ioParser).map(c =>
  ...
  val createIndex = P("create" ~ "index" ~/ strOrVar).map(c =>
    Instruction("createIndex", interceptFunction.createIndex, Seq(c)))
  val getMapping = P(strOrVar ~ "mapping").map(c =>
  ...
  val instrument: P[Instruction] = P(
    (health | shutdown | clusterStats | indicesStats | nodeStats | pendingTasks | waitForStatus
