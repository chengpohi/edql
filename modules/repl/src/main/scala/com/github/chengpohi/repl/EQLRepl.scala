...
object EQLRepl extends EQLConfig
  with EQLContext
  with JSONOps {
  val ANSI_GREEN = "\u001B[32m"
  val ANSI_RESET = "\u001B[0m"

  val eqlInterpreter = new EQLInterpreter()
  val scriptRunner = new EQLScriptRunner(eqlInterpreter)

  def interpret(line: String): Reader[EQLInterpreter, String] = {
    Reader((interpreter: EQLInterpreter) => interpreter.run(line).beautify)
  }
